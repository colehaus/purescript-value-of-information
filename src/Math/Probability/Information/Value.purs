...
import Math.Probability.Information.Value.Utility (unsafeFromJustBecause, unsafeFromRightBecause, unsafeLookupBecause)
...
investigationAndDecisionTreeValueX ::
  ...
  -> ValuationRule prob n
  -> InvestigationAndDecisionTree prob finding choice result
  -> Dist prob (Tagged Value n)
investigationAndDecisionTreeValueX project decide value =
  investigate <<< IDT.unmake
  where
    investigate idt = do
      Tuple _ sdt <- idt
      pure $ simpleDecisionTreeValue project decide value sdt
